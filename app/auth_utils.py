...
refresh_exp = int(getenv("REFRESH_TOKEN_EXPIRE_DAYS"))
access_exp = int(getenv("ACCESS_TOKEN_EXPIRE_MINUTES"))
...
def create_refresh_token(data: dict, redis: redis.asyncio.Redis) -> str:
    refresh_token = create_token({**data, "type":"refresh"}, timedelta(days=refresh_exp))
    # The refresh token must be stored in redis.
    user_id = data.get("sub")
    redis.set(f"session:{user_id}", refresh_token, timedelta(days=refresh_exp))
